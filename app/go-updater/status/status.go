...
type (
	UpdateStatus struct {
		...
		UpdateStart time.Time `json:"updateStart"`
...
func (su *StatusUpdater) GetStatus(ctx context.Context) (*UpdateStatus, error) {
	...
	go func() {
		...
		c := su.status
		c.Errors = make([]json.RawMessage, len(c.Errors))
...
func (su *StatusUpdater) AddError(ctx context.Context, newErr error) error {
	return su.withUpdate(ctx, func(us UpdateStatus) UpdateStatus {
		if us.IsUpdating {
			...
			d, err := json.Marshal(newErr)
			if err == nil {
				rawError = d
			} else {
				rawError, _ = json.Marshal(newErr.Error())
			}

			us.Errors = append(us.Errors, rawError)
		}
		return us
	})
}
